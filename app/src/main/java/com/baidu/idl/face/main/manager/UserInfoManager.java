...
public class UserInfoManager {
    ...
    public void updateImage(final Bitmap bitmap, final String groupId, final String userName,
                            final String imageName, final UserInfoListener listener, final String card, final String pwd) {
        mExecutorService.submit(new Runnable() {
            @Override
            public void run() {
                if(listener == null) {
                ...
                if (ret == -1) {
                    ...
                    // 添加用户信息到数据库
                    boolean update = FaceApi.getInstance().userUpdate(groupId, userName, imageName, bytes,card,pwd);
                    if (update) {
                        // 保存图片到新目录中
