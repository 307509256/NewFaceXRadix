...
import static com.baidu.idl.main.facesdk.model.BDFaceSDKCommon.BDFaceLogInfo.BDFACE_LOG_VALUE_MESSAGE;
...
public class FaceSDKManager {
    ...
    private FaceSDKManager() {
        ...
        faceAuth.setActiveLog(BDFACE_LOG_VALUE_MESSAGE);
...
//        faceLiveness = new FaceLive();
    ...
    public void initModel(final Context context, final SdkInitListener listener) {
        ...
        faceDetect.initFaceClose(context, GlobalSet.EyeClose_Model, GlobalSet.MouthClose_Model, new Callback() {
            @Override
            public void onResponse(int code, String response) {
                if (code != 0 && listener != null) {
                    listener.initModelFail(code, response);
    ...
    public boolean initConfig() {
        if (faceDetect != null) {
            ...
            config.isEyeClose = false;
            config.isMouthClose = false;
...
//                    Log.e("shangtest", "mouthclose:" + faceInfos[0].mouthclose +
//                            "  leftEyeclose:" + faceInfos[0].leftEyeclose +
//                            "  rightEyeclose:" + faceInfos[0].rightEyeclose);
...
//                     避免RGB检测关键点在IR对齐活体稳定，增加红外检测
                    ...
                    FaceInfo[] faceInfosIr = faceDetectNir.detect(BDFaceSDKCommon.DetectType.DETECT_NIR, nirInstance);
