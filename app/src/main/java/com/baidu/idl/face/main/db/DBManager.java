...
public class DBManager {
     ...
     * 查询用户（根据dbId）
     */
    public List<User> queryUserByCardNo(String cardNo) {
        List<User> users = new ArrayList<>();
        Cursor cursor = null;
        try {
            if (mDBHelper == null) {
    ...
    public List<User> queryUserByUserID(String user_Id) {
        List<User> users = new ArrayList<>();
        Cursor cursor = null;
        try {
            if (mDBHelper == null) {
     ...
     * 更新用户
     */
    public boolean updateUser(String groupId, String userName, String imageName, byte[] feature, String card, String pwd) {
    ...
    public boolean deleteUserALL() {
        ...
        try {
            mDatabase = mDBHelper.getWritableDatabase();
            beginTransaction(mDatabase);
            if (mDatabase.delete(DBHelper.TABLE_USER, null, null) < 0) {
                return false;
            }
            setTransactionSuccessful(mDatabase);
            success = true;
