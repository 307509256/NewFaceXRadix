...
public class FaceApi {
    ...
    public User getUserByCardNo(String card) {
        ...
        List<User> userList = DBManager.getInstance().queryUserByCardNo(card);
        if (userList != null && userList.size() > 0) {
            return userList.get(0);
        }
        return null;
    ...
    public List<User> getUserListByCardNo(String card) {
        if (TextUtils.isEmpty(card)) {
            return null;
        }
        List<User> userList = DBManager.getInstance().queryUserByCardNo(card);
     ...
     * 更新用户
     */
    public boolean userUpdate(String groupId, String userName, String imageName, byte[] feature, String card, String pwd) {
        if (groupId == null || userName == null || imageName == null || feature == null || card == null || pwd == null) {
            return false;
        }

        boolean ret = DBManager.getInstance().updateUser(groupId, userName, imageName, feature, card, pwd);
        return ret;
    }
    ...
    public boolean userDeleteById(String userId) {
        if (TextUtils.isEmpty(userId)) {
            return false;
        }

        boolean ret = DBManager.getInstance().deleteUserById(userId);
        return ret;
    }
    ...
    public boolean registerUserIntoDBmanager(String groupName, String userName,String picName,
                                             String userInfo, byte[] faceFeature, String card, String pwd) {
                ...
                if (listGroup != null && listGroup.size() > 0) {
                    ...
                    for (int i = 0; i < listGroup.size(); i++) {
                        ...
                        for (int j = 0; j < listUser.size(); j++) {
                            ...
                            if (listUser.get(j).getFeature() != null) {
